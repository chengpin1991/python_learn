...
class HTTP:
    ...
    def Session(self,expires=15*60):
        ...
        if hasattr(self,"sessionObj"):
            ...
            self.sessionObj = self.config.make_session_object(self.config,
                expires)
    ...
    def save_session(self):
        ...
        if hasattr(self,"sessionObj"):
            ...
            if self.sessionObj.closed():
                self.ns['SET_COOKIE']["sessionId"] = self.sessionObj._id
                self.ns['SET_COOKIE']["sessionId"]["path"] = '/'
                self.ns['SET_COOKIE']["sessionId"]["max-age"] = 0
                self.ns['SET_COOKIE']["sessionId"]["expires"] = 0
