...
def init(server_config):
    ...
    config[None].startup_modules = []
    config[None].shutdown_modules = []
    ...
    if not os.path.exists(hosts_file):
        ...
        for line in open(hosts_file):
            ...
            set_host_conf(host,conf_file, True)
    ...
    #print "names =", names
    for name in names:
        set_host_conf(name,localhost_script, False)
...
def set_host_conf(host,conf_file,run_startup):
    already_in_config = host in config
    # set host-specific configuration
    ...
    if (not run_startup) and (not already_in_config) :
        config[host].startup_modules = []
        config[host].shutdown_modules = []
