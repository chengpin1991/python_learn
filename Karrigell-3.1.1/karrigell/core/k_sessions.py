...
class SessionObject(object):
    ...
    def close(self):
        self._expires = None #datetime.timedelta(0, 0, 0)
        for name in dir(self):
            if not name in dir(SessionObject) +\
                ['_expires','_timestamp','_id']:
                delattr(self,name)
...
def save_session_object_memory(config, session_obj):
    config.rlock.acquire()
    if session_obj.closed(): # closed session
        del config.session_objects[session_obj._id]
    else:
...
def get_session_object_persistent(config,session_id,expires):
    config.rlock.acquire()
    _path = os.path.join(config.data_dir,"sessions",session_id)
    try:
        session_obj = cPickle.load(open(_path,"rb"))
    except IOError:
        session_obj = SessionObject(expires,session_id)
        save_session_object_persistent(config,session_obj)
    config.rlock.release()
